...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    if [ $PEER -eq 0 ]; then
        ...
        export CORE_PEER_ADDRESS=peer${PEER}.org${USING_ORG}.example.com:7051
    elif [ $PEER -eq 1 ]; then
        export CORE_PEER_LOCALMSPID="Org1MSP"
        export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG2_CA
        export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
        export CORE_PEER_ADDRESS=peer${PEER}.org${USING_ORG}.example.com:9051
    else
        export CORE_PEER_LOCALMSPID="Org1MSP"
        export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG3_CA
        export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
        export CORE_PEER_ADDRESS=peer${PEER}.org${USING_ORG}.example.com:8051
    fi
    ...
    export CORE_PEER_ADDRESS=peer${PEER}.org${USING_ORG}.example.com:11051
  else
    errorln "ORG Unknown"
  fi
